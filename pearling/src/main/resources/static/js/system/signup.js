...
window.addEventListener("load", function () {
  ...
  pwdInput.addEventListener("input", function () {
    ...
    if (validatePassword(pwd)) {
      ...
      displayPasswordErrorMessage(
        "비밀번호는 영문, 숫자, 특수 기호를 포함하여 6자리 이상이어야 합니다."
      );
    }
  });
  ...
  memberAddBtn.addEventListener("click", async function (e) {
    ...
    let isEmailConfirmed = await verifyEmailConfirmation(
      sentCode,
      emailConfirmation
    );
    ...
    try {
      ...
      let signupConfirmYes = signupConfirmModal.querySelector(
        ".signup-confirm-yes"
      );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  checkEmailBtn.addEventListener("click", async function () {
    ...
    let fullEmail =
      memail.value + "@" + document.getElementById("memail-domain").value;
